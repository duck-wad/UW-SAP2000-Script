...
def generate_stringer_nodes(length, num_segments, y_pos, z_pos_top, depth,
                            web_divisions, web_offset):
    ...
    for i in range(num_segments):
        ...
        for j in range(web_divisions):
            web_points.append(
                ((web_offset + i * segment_length + j * web_div_length, y_pos,
                  z_pos_top - depth),
            ...
            web_points.append(
                ((web_offset + i * segment_length + (j + 0.5) * web_div_length,
                 ...
                 (web_offset + i * segment_length + (j + 1.0) * web_div_length,
                  y_pos, z_pos_top - depth)))

        vertical_web_points.append(
            ((web_offset + i * segment_length, y_pos, z_pos_top),
             (web_offset + i * segment_length, y_pos, z_pos_top - depth)))
        vertical_web_points.append(
            ((i * segment_length + segment_length - web_offset, y_pos,
              z_pos_top), (i * segment_length + segment_length - web_offset,
                           y_pos, z_pos_top - depth)))
...
def generate_moment_frame_nodes(width, x_pos, z_pos_top, depth, web_divisions,
    ...
    moment_chord_points.append(
        ((x_pos, 0.0, z_pos_top), (x_pos, width, z_pos_top)))
    moment_chord_points.append(
    ...
    for i in range(web_divisions):
        ...
        web_points.append(
            ((x_pos, web_offset + (i + 0.5) * web_div_length, z_pos_top),
             (x_pos, web_offset + (i + 1.0) * web_div_length,
              z_pos_top - depth)))

    vertical_web_points.append(((x_pos, web_offset, z_pos_top),
                                (x_pos, web_offset, z_pos_top - depth)))
    vertical_web_points.append(
        ((x_pos, width - web_offset, z_pos_top), (x_pos, width - web_offset,
                                                  z_pos_top - depth)))
