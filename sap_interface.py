...
def sap_create_members(sap_model, members):
    ...
    for member in members:
        name = sap_model.FrameObj.AddByCoord(*member.start, *member.end, 'foo',
                                             member.section)[0]
        ...
        if member.start_restraint != None:
            ret = sap_model.PointObj.SetRestraint(start_point,
                                                  member.start_restraint)
        if member.end_restraint != None:
            ret = sap_model.PointObj.SetRestraint(end_point,
                                                  member.end_restraint)
...
def sap_create_stringer(sap_model, members, vertical_loads, L1_value,
                        L2_value):
    starting_point = members[0].start
    ending_point = members[-1].end
    stringer_length = ending_point[0] - starting_point[0]

    frame, ret = sap_model.FrameObj.AddByCoord(*starting_point, *ending_point,
                                               'foo', members[0].section)
    for load in vertical_loads:
        ...
        ret = sap_model.FrameObj.SetLoadDistributed(frame,
                                                    load['Name'],
                                                    1,
                                                    10,
                                                    L1_start_relative,
    ...
    _, stringer_frames, ret = sap_model.EditFrame.DivideAtIntersections(
        frame, 0, [])
